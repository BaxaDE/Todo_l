...
class HomePageView(LoginRequiredMixin, View):
    def get(self, request):
        ...
        if search_query:
            todo_items = todo_items.filter(body__icontains=search_query)
...
class TodoItemEdit(View):
    ...
    def post(self, request, id):
        todo_item = Todo.objects.get(id=id)
        form = TodoItemForm(instance=todo_item, data=request.POST)
        if form.is_valid():
            form.save()
...
class TodoItemsByCategory(LoginRequiredMixin, View):
    def get(self, request, slug):
        category = Category.objects.get(slug=slug)
        todo_items = Todo.objects.filter(Q(user=request.user) & Q(category=category)).order_by('-id')
